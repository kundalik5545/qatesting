...
export async function generateStaticParams() {
  const postsDirectory = path.join(process.cwd(), "Blog/JavascriptBlog");
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${basicDetails.websiteURL}/javascript/${slug}`,
      images: data.image ? [{ url: data.image }] : [],
    },
    alternates: {
      canonical: `${basicDetails.websiteURL}/javascript/${slug}`,
    },
  };
}

// Blog Post Component
const JsPage = async ({ params }) => {
  const { slug } = params;
  const filePath = path.join(
    process.cwd(),
    "Blog/JavascriptBlog",
